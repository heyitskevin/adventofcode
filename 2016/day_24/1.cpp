...
int find_fewest_steps(vector<vector<char> > grid) {
            ...
            if (mask & (1 << i)) {
                for (int j = 0; j < n ; j++) {
                    if(!(mask & (1 << j))) {
                        ...
                        // cout << dp[next_mask][j] << " " << dp[mask][i] + adjacency_matrix[i][j] << endl;
                        dp[next_mask][j] = min(dp[next_mask][j], dp[mask][i] + adjacency_matrix[i][j]);
                    }
                }
            }
        }
    ...
    for (int i = 0; i < n; i++) {
        if (i != 0) {
            min_dist = min(min_dist, dp[all_visited][i]);
        }
    }
    cout << "DONE" << endl;;
    return min_dist;
